...
You can use the arrow keys (`↑` and `↓`) or your mouse to select a suggestion. You can combine arrow keys with `Ctrl` to jump through the block of history or tag suggestions.

When you click or press `Enter` on a search history suggestion, you are immediately taken to the search results page for that history item. To avoid submitting the search for a history item, use `Shift + Enter` or `Shift + LMB (click)` instead.

When you click or press `Enter` on a regular (non-history) tag suggestion, the tag will be appended to the search input without submitting the search query. This allows you to continue building a complex search query step by step. If you want to accept a regular tag suggestion and immediately search for it, then use `Ctrl + Enter` or `Ctrl + LMB (click)` when selecting it.
